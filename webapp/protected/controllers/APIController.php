...
class ApiController extends Controller {
    ...
    private $config = array(
        ...
        //valid creation requests, the key should be the request type and the value the Model object to create
        'document'        => array(
            'class' => 'Document',
            'type' => 'record'
        ),
    ...
    public function actionList() {
        ...
        switch ($_GET['model']) {
            ...
            default:
                ...
                //add any filters
                if (isset($safe_model_requests[$request_model]['filter'])) {
                    foreach ($safe_model_requests[$request_model]['filter'] as $filter_by) {
                        if (isset($_GET[$filter_by])) {
                            $criteria->condition = $filter_by . ' = ' . $_GET[$filter_by];
                        }
                    }
                }
                ...
                if (array_key_exists($request_model, $safe_model_requests)) {
                    ...
                    $count  = $safe_model_requests[$request_model]['class']::model()->count($criteria);
                } else {
        ...
        if (is_null($models)) {
            $this->_sendErrorResponse(200, sprintf('No items where found for model %s', $_GET['model']));
        } else {
            ...
            foreach ($models as $model) {
                ...
                //add extra attributes devined via a method call
                if (isset($safe_model_requests[$request_model]['extra_attributes'])) {
                    $extra_attributes = $safe_model_requests[$request_model]['extra_attributes'];

                    if (is_array($extra_attributes) && count($extra_attributes) > 1) {
                        foreach ($extra_attributes as $name => $value) {
                            $attributes[$name] = $model->$value();
                        }
                    }
                }
                $rows[] = $attributes;
            ...
            $this->_sendResponse(200, CJSON::encode(array('total' => $count, 'success' => true, 'data' => $rows)));
        }
    }
    ...
    public function actionView() {
        ...
        //check if we can create a model of this type
        if (array_key_exists($request_model, $this->config)) {
        ...
        if (is_null($request_model)) {
            $this->_sendErrorResponse(404, 'No Item found with id ' . $_GET['id']);
        } else {
            $attributes = array();
            //add extra attributes devined via a method call
            if (isset($safe_model_requests[$request_model]['extra_attributes'])) {
            ...
            $this->_sendResponse(200, CJSON::encode(array(array_merge($attributes, $model->attributes))));
        }
    }
    ...
    private function getTreeEncode($models, $follow = true) {
        ...
        foreach ($models as $model) {
            ...
            $rows[]  = array("text" => $model->getDisplayName(), "icon" => $model->getIcon(),"object_type" => get_class($model), "object_id" => $model->id, "id" => get_class($model) . '-' . $model->id, "loaded" => $isLeafe , "cls" => "", "disabled" => true, "order" => $model->getOrder());
        }//"expanded"
